...
"""
    ...
    Classes/Functions:
        - parse_gitignore(gitignore_path: Path) -> Set[str] (line 62)
        - is_path_ignored(path: Path, ignore_patterns: Set[str], root_dir: Path) -> bool (line 87)
        - load_blacklist_whitelist(directory: Path) -> Tuple[Set[str], Set[str]] (line 111)
        - should_process_file(file_path: Path, root_dir: Path, ignore_patterns: Set[str], blacklist: Set[str], whitelist: Set[str]) -> bool (line 150)
        - _get_header_content_lines(classes: List[ClassInfo], functions: List[SignatureInfo], language: str, line_offset: int, manual_content: str | None = None, placeholder: bool = False) -> List[str] (line 220)
        - _format_header(classes: List[ClassInfo], functions: List[SignatureInfo], language: str, line_offset: int, manual_content: str | None = None, placeholder: bool = False) -> str (line 295)
        - get_preserved_header_end_line(lines: List[str], language: str) -> int (line 317)
        - _extract_docstring_and_code(code_body: str, language: str) -> tuple[str | None, str] (line 415)
        - process_file(path: Path, verbose: bool = False, beta: bool = False) -> None (line 452)
        - discover_and_process_files(paths: List[str], verbose: bool = False, beta: bool = False) -> None (line 576)
...
def _get_header_content_lines(
    ...
    if manual_content:
        ...
        if language == "python":
            ...
            lines.extend(manual_content.splitlines())
        else:
            if style.prefix.strip():
                lines.append(style.prefix.rstrip()) # Add a clean separator like ' *'
            lines.extend(
                f"{style.prefix}{line}" for line in manual_content.splitlines()
            )
