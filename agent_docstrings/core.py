...
def _normalize_content_for_version_comparison(content: str) -> str:
    """
    Normalize content for version comparison by removing version-specific information.
    This allows us to compare content ignoring only version differences.
    """
    # * Replace version string with a placeholder to enable version-independent comparison
    version_pattern = r"Table of content is automatically generated by Agent Docstrings v[\d\.]+"
    normalized = re.sub(version_pattern, "Table of content is automatically generated by Agent Docstrings v{VERSION}", content)
...
def process_file(path: Path, verbose: bool = False, beta: bool = False) -> None:
    ...
    try:
        ...
        # * Check if content changed, ignoring version differences
        # Only write changes if content changed (excluding version-only changes)
        original_normalized = _normalize_content_for_version_comparison(original_content)
        new_normalized = _normalize_content_for_version_comparison(new_content)
        ...
        if new_normalized != original_normalized:
            ...
            print(f"No changes for {language.capitalize()}: {path} (version-only change skipped)")
