...
"""
    ...
    Classes/Functions:
        - parse_gitignore(gitignore_path: Path) -> Set[str] (line 55)
        - is_path_ignored(path: Path, ignore_patterns: Set[str], root_dir: Path) -> bool (line 80)
        - load_blacklist_whitelist(directory: Path) -> Tuple[Set[str], Set[str]] (line 104)
        - should_process_file(file_path: Path, root_dir: Path, ignore_patterns: Set[str], blacklist: Set[str], whitelist: Set[str]) -> bool (line 143)
        - _get_header_content_lines(classes: List[ClassInfo], functions: List[SignatureInfo], language: str, line_offset: int) -> List[str] (line 213)
        - _format_header(classes: List[ClassInfo], functions: List[SignatureInfo], language: str, line_offset: int) -> str (line 259)
        - get_preserved_header_end_line(lines: List[str], language: str) -> int (line 279)
        - process_file(path: Path, verbose: bool = False, beta: bool = False) -> None (line 377)
        - discover_and_process_files(paths: List[str], verbose: bool = False, beta: bool = False) -> None (line 535)
