...
"""
    ...
    Classes/Functions:
        - parse_gitignore(gitignore_path: Path) -> Set[str] (line 60)
        - is_path_ignored(path: Path, ignore_patterns: Set[str], root_dir: Path) -> bool (line 85)
        - load_blacklist_whitelist(directory: Path) -> Tuple[Set[str], Set[str]] (line 109)
        - should_process_file(file_path: Path, root_dir: Path, ignore_patterns: Set[str], blacklist: Set[str], whitelist: Set[str]) -> bool (line 148)
        - _get_header_content_lines(classes: List[ClassInfo], functions: List[SignatureInfo], language: str, line_offset: int, manual_content: str | None = None, placeholder: bool = False) -> List[str] (line 218)
        - _format_header(classes: List[ClassInfo], functions: List[SignatureInfo], language: str, line_offset: int, manual_content: str | None = None, placeholder: bool = False) -> str (line 293)
        - get_preserved_header_end_line(lines: List[str], language: str) -> int (line 315)
        - _extract_docstring_and_code(code_body: str, language: str) -> tuple[str | None, str] (line 413)
        - process_file(path: Path, verbose: bool = False, beta: bool = False) -> None (line 450)
